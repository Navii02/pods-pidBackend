...
const AddTag = async (req, res) => {
  let connection;
  try {
    const { tagNumber, parentTag, name, type, model } = req.body;
    ...
    await connection.query(
      `INSERT INTO Tags (tagId, number, name, parenttag, type, filename)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [tagId, tagNumber, name, parentTag || null, type, model || null]
    );

    // Insert into TagInfo table
    await connection.query(
      `INSERT INTO TagInfo (tagId, tag, type)
       VALUES (?, ?, ?)`,
      [tagId, name, type]
    );
    ...
    if (type.toLowerCase() === "line") {
      await connection.query(
        `INSERT INTO LineList (tagId, tag)
         VALUES (?, ?)`,
        [tagId, name]
      );
    } else if (type.toLowerCase() === "equipment") {
      await connection.query(
        `INSERT INTO EquipmentList (tagId, tag)
         VALUES (?, ?)`,
        [tagId, name]
      );
    } else if (type.toLowerCase() === "valve") {
      await connection.query(
        `INSERT INTO ValveList (tagId, tag, name)
         VALUES (?, ?)`,
        [tagId, name]
      );
    }
